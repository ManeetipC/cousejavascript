  ...
  componentDidMount() {
    ...
    let dd = fetch('https://api.coindesk.com/v1/bpi/currentprice.json').then((resp) => resp.json())
      ...
      .then(() => {
        ...
        let dd = fetch('https://api.coindesk.com/v1/bpi/historical/close.json?currency=USD').then((resp) => resp.json())
          .then(data => {
            let datapush = []
      ...
      <div className="App" style={{ margin: 150 }}>
        ...
        <br />
        {
          this.state.flagspin ? <LineChart width={730} height={250} data={this.state.data}
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" name={`${this.state.name}`} dataKey="price" stroke="#8884d8" />
